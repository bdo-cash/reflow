...
//    static final TAG.LogTag TAG = new TAG.LogTag(Dependency.class.getName());
//
//    static class Basis {
//        final List<Trait<?>> traits = new LinkedList<>();
//        // 表示每个任务结束的时候应该为后面的任务保留哪些Key$(transform后的)
//        // 注意：可能get出来为null, 表示根本不用输出。
//        final Map<String, Map<String, Key$>> dependencies = new HashMap<>();
//        // 任务的输出经过转换, 生成最终输出传给下一个任务。
//        final Map<String, Set<Transformer>> transformers = new HashMap<>();
//        // 可把前面任意任务的输出作为输入的全局转换器。
//        final Map<String, Set<Transformer>> transGlobal = new HashMap<>();
//        // 虽然知道每个任务有哪些必要的输出, 但是整体上这些输出都要保留到最后吗?
//        Map<String, Set<Key$>> outsFlowTrimmed;
//        Set<Key$> outs;
//
//        int steps() {
//            return traits.size();
//        }
//
//        int stepOf(Trait trait) {
//            int step = traits.indexOf(trait);
//            if (step < 0) for (Trait tt : traits)
//                if (tt instanceof Trait.Parallel
//                        && ((Trait.Parallel) tt).traits().indexOf(trait) >= 0) {
...
//        }
//
//        Trait first(boolean child) {
//            return first$last(true, child);
//        }
//
//        Trait last(boolean child) {
//            return first$last(false, child);
//        }
//
//        private Trait first$last(boolean first$last, boolean child) {
//            if (traits.size() == 0) return null;
//            else {
//                final Trait trait = traits.get(first$last ? 0 : traits.size() - 1);
//                if (child && trait instanceof Trait.Parallel) {
//                    return first$last ? ((Trait.Parallel) trait).first() : ((Trait.Parallel) trait).last();
//                }
//                return trait;
//            }
//        }
...
//    // Key$是transform后的
//    private final Map<String, Map<String, Key$>> useless = new HashMap<>();
//    private final Map<String, Key$> inputRequired = new HashMap<>();
//
//    Dependency() {
//    }
//
...
//     * 给前面最后添加的任务增加并行任务。
//     *
//     * @param trait 新增的任务具有的特性。
//     * @return 当前依赖组装器。
//     */
//    public Dependency with(Trait<?> trait) {
//        assertf(!(requireNonNull(trait) instanceof Trait.Parallel));
//        requireTaskNameDifferent(trait, names);
//        if (basis.traits.isEmpty()) {
//            basis.traits.add(trait);
//        } else {
//            final Trait<?> last = basis.last(false);
//            final Collection<Trait<?>> traits;
//            if (last instanceof Trait.Parallel) {
//                traits = ((Trait.Parallel) last).traits();
//            } else {
//                final Trait.Parallel parallel = new Trait.Parallel(last);
//                basis.traits.remove(last);
//                basis.traits.add(parallel);
...
//     * @return 当前依赖组装器。
//     */
//    public Dependency then(Trait<?> trait) {
//        assertf(!(requireNonNull(trait) instanceof Trait.Parallel));
//        requireTaskNameDifferent(trait, names);
//        final Trait<?> last = basis.last(false);
//        if (last != null) genIOPrev(last, null, basis, inputRequired, useless);
//        basis.traits.add(trait);
//        return this;
//    }
//
...
//     * @return 当前依赖组装器。
//     */
//    public Dependency then(Dependency dependency) {
//        if (basis.traits.isEmpty()) {
//            copy(dependency);
//        } else {
//            for (Trait<?> trait : dependency.basis.traits) {
...
//                            first = false;
//                        } else {
//                            with(tt).transition$(dependency.basis.transformers.get(tt.name$()), false);
//                        }
//                    }
//                } else {
//                    then(trait).transition$(dependency.basis.transformers.get(trait.name$()), false);
//                }
//                then$(dependency.basis.transGlobal.get(trait.name$()), false);
//            }
//        }
//        return this;
//    }
...
//     * @see #transition(Set)
//     */
//    public Dependency transition(Transformer trans) {
...
//     * @see Transformer
//     */
//    public Dependency transition(Set<Transformer> tranSet) {
//        return transition$(tranSet, true);
//    }
//
