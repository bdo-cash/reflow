...
object In {
  ...
  private def generate(map: Map[String, Any]): Set[KvTpe[_ <: AnyRef]] = if (map.isEmpty) Set.empty else
    (new mutable.HashSet[KvTpe[_ <: AnyRef]] /: map) { (set, kv) => set += new KvTpe(kv._1, kv._2.getClass, 0, true) {} }

  private class M private[reflow](keys: Set[KvTpe[_ <: AnyRef]], map: Map[String, Any],
