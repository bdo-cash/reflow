...
/**
  ...
  */
class Out private[reflow](map: Map[String, Kce[_]]) {
  private[reflow] def this() = this(Map.empty[String, Kce[_]])

  private[reflow] def this(keys: Set[Kce[_]]) = this((new mutable.AnyRefMap[String, Kce[_]] /: keys) { (m, k) => m += (k.key, k) })

  // 仅读取
  private[reflow] val _keys = map.toMap[String, Kce[_]]
  // 由于并行的任务，不可能有相同的key, 没有必要让本类的整个方法调用都进行sync, 因此用并行库是最佳方案。
  private[reflow] val _map = new concurrent.TrieMap[String, Any]
  private[reflow] val _nullValueKeys = new concurrent.TrieMap[String, Kce[_]]
  ...
  /**
    ...
    */
  private[reflow] def putWith(map: Map[String, Any], nulls: Map[String, Kce[_]],
  ...
  /**
    ...
    * @return
    */
  def get[T](key: Kce[T]): Option[T] = Option(key.takeValue(_map))
  ...
  /**
    ...
    * @return
    */
  def keysDef(): immutable.Set[Kce[_]] = _keys.values.toSet
  ...
  /**
    ...
    * @return
    */
  def keys(): immutable.Set[Kce[_]] = _keys.values.filter { k => _map.contains(k.key) || _nullValueKeys.contains(k.key) }.toSet
